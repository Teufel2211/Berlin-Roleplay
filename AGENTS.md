...
Guidance for AI agents working on this repository: **Notruf Hamburg** – Discord-Bot + Website-Dashboard.
...
- All embeds and bot messages are **German**; footer: `🏥 Notruf Hamburg • <Server-Name>`.
- Slash commands: `/verify`, `/warteraum`, `/giveaway`, `/counting`, `/bewerbung`, `/ticket`, `/admin`.
...
| `DASHBOARD_PASSWORD_HASH` | Generated by `npm run setup` |
| `SESSION_SECRET` | Session signing |
| `WEB_PORT` | Dashboard port (default 3000) |

Setup order: fill `.env` → `npm run migrate` → `npm run setup` → `npm run deploy` → `npm start`.
