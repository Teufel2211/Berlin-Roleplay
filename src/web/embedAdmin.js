...
function assertDiscordToken() {
  if (!config.discordToken) throw new Error('Discord-Bot-Token fehlt in Vercel. Setze DISCORD_TOKEN als Production Environment Variable.');
}
...
function buildComponents(embedId, buttons) {
  ...
  for (let i = 0; i < Math.min(buttons.length, 25); i++) {
    const btn = buttons[i]; const style = Number(btn.style || 1); const isLink = style === BUTTON_STYLES.link;
    const b = new ButtonBuilder().setLabel(String(btn.label || 'Button').slice(0, 80)).setStyle(isLink ? 5 : style >= 1 && style <= 4 ? style : 1);
    if (btn.emoji) { try { b.setEmoji(String(btn.emoji).slice(0, 100)); } catch (_) {} }
    if (isLink) b.setURL(String(btn.url || 'https://example.com').slice(0, 2048)); else b.setCustomId(`emb_${embedId}_${i}`);
    row.addComponents(b); if (row.components.length === 5) { rows.push(row); row = new ActionRowBuilder(); }
  }
...
function collect(body) {
  ...
  for (let i = 0; i < labels.length; i++) { const label = String(labels[i] || '').trim(), style = Number(styles[i] || 1), url = String(urls[i] || '').trim(); if (!label || (style === 5 && !url)) continue; data.buttons.push({ label, style, emoji: String(emojis[i] || '').trim(), url, action: String(actions[i] || 'none'), action_value: String(actionValues[i] || '').trim() }); }
  return data;
}
async function saveRow(guildId, name, data, channelId, editId) {
  data.name = name;
  if (editId) { const { data: existing } = await withRetry(() => getClient().from(TABLES.embeds).select('*').eq('id', editId).eq('guild_id', guildId).maybeSingle()); if (!existing) throw new Error('Embed nicht gefunden.'); const payload = { name, data, updated_at: new Date().toISOString() }; if (channelId) payload.channel_id = channelId; const { data: updated } = await withRetry(() => getClient().from(TABLES.embeds).update(payload).eq('id', editId).eq('guild_id', guildId).select().single()); return updated; }
  const { data: inserted } = await withRetry(() => getClient().from(TABLES.embeds).insert({ guild_id: guildId, name, data, channel_id: channelId || null }).select().single()); return inserted;
}
async function handleAction(req, res) {
  const guildId = req.guildId, body = req.body || {}, action = body.action, name = String(body.name || '').trim(), editId = body.edit_id ? Number(body.edit_id) : null, redirect = `/dashboard/servers/${guildId}/feature/embeds`, flash = (msg) => res.redirect(`${redirect}?msg=${encodeURIComponent(msg)}`);
  try {
    if (!name && action !== 'delete') return flash('Bitte einen Namen angeben.'); const data = collect(body), channelId = String(body.channel || '').trim();
    if (action === 'delete') { if (editId) { const { data: row } = await withRetry(() => getClient().from(TABLES.embeds).select('*').eq('id', editId).eq('guild_id', guildId).maybeSingle()); if (row) { await deleteMessage(row); await withRetry(() => getClient().from(TABLES.embeds).delete().eq('id', editId).eq('guild_id', guildId)); await auditService.log(guildId, req.session.user.tag, 'embeds.delete', { name: row.name }); } } return flash('Embed gelöscht.'); }
    if (action === 'save') { const row = await saveRow(guildId, name, data, channelId, editId); await auditService.log(guildId, req.session.user.tag, 'embeds.save', { name: row.name }); return flash(`Embed „${row.name}“ gespeichert.`); }
    if (action === 'post' || action === 'update' || action === 'repost') { if (!channelId && !editId) return flash('Bitte einen Kanal wählen.'); const row = await saveRow(guildId, name, data, channelId || undefined, editId); try { if (action === 'update' && row.message_id && row.channel_id && (channelId === row.channel_id || !channelId)) await editEmbed(row); else await postEmbed(row); await auditService.log(guildId, req.session.user.tag, `embeds.${action}`, { name: row.name, channel: row.channel_id }); return flash(action === 'update' ? 'Embed aktualisiert.' : 'Embed gepostet.'); } catch (err) { logger.error(`Embed-Aktion Discord fehlgeschlagen: ${err.stack || err.message}`); return flash(`Fehler: ${err.message}`); } }
    return flash('Unbekannte Aktion.');
  } catch (err) { logger.error(`Embed-Aktion fehlgeschlagen: ${err.stack || err.message}`); return flash(`Fehler: ${err.message}`); }
}
module.exports = { handleAction, parseColor, buildEmbed, buildComponents, parseData, normalizeStoredData };
