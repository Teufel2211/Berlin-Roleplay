...
function buildComponents(embedId, buttons) {
  ...
  for (let i = 0; i < buttons.length; i++) {
    ...
    if (btn.emoji) {
      try { b.setEmoji(String(btn.emoji).slice(0, 100)); } catch (err) { /* Emoji ungültig */ }
    }
    if (isLink) {
      b.setURL(String(btn.url || 'https://example.com'));
    } else {
      b.setCustomId(`emb_${embedId}_${i}`);
    }
    row.addComponents(b);
...
async function saveRow(guildId, name, data, channelId, editId) {
  ...
  const { data: inserted } = await withRetry(() =>
    getClient().from(TABLES.embeds).insert({ guild_id: guildId, name, data, channel_id: channelId || null }).select().single()
  );
  return inserted;
}
  ...
  try {
    ...
    if (action === 'post' || action === 'update' || action === 'repost') {
      ...
      try {
        ...
        logger.error(`Embed posten fehlgeschlagen: ${err.stack || err.message}`);
        return flash(`Fehler beim Posten: ${err.message}`);
      }
    }
