...
function createApp() {
  ...
  app.get('/dashboard/servers/:guildId/feature/:feature', async (req, res, next) => {
    ...
    if (feature.id === 'overview') { const [tickets, giveaways] = await Promise.all([getClient().from(TABLES.tickets).select('id', { count: 'exact' }).eq('guild_id', gid).in('status', ['open', 'offen']), getClient().from(TABLES.giveaways).select('id', { count: 'exact' }).eq('guild_id', gid).eq('ended', false).gt('ends_at', new Date().toISOString())]); const { client: discordClient } = require('../discord/client'); const botInstalled = discordClient?.isReady() ? discordClient.guilds.cache.has(gid) : true; return res.render('server', { ...base, data: { ...sdata, stats: { tickets: tickets.count || 0, giveaways: giveaways.count || 0 }, botInstalled } }); }
