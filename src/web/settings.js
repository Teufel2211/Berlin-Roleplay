...
async function sendPanel(guildId, channelId, userTag) {
  ...
  const r = await fetch(`https://discord.com/api/v10/channels/${channelId}/messages`, { method:'POST', headers:{ Authorization:`Bot ${config.discordToken}`, 'Content-Type':'application/json' }, body:JSON.stringify(payload) }); if(!r.ok) throw new Error(`Discord API ${r.status}: ${await r.text()}`); const msg=await r.json(); if(panel){await withRetry(() => getClient().from(TABLES.ticketPanels).update({ message_id: String(msg.id), channel_id: String(channelId) }).eq('id', panel.id));}else{await withRetry(() => getClient().from(TABLES.ticketPanels).insert({ guild_id: guildId, name: 'Standard Panel', channel_id: String(channelId), message_id: String(msg.id), enabled: true }));} await auditService.log(guildId,userTag,'ticket.panel.send',{channelId,messageId:msg.id}); return msg.id;
}
