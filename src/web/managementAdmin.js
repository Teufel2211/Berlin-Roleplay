...
async function handleAction(req, res) {
  ...
  try {
    ...
    if (feature === 'calendar' && body.action === 'add_event' && String(body.title || '').trim() && body.starts_at) {
      await teamService.createEvent(guildId, { title: String(body.title).trim(), description: String(body.description || '').trim() || null, starts_at: new Date(body.starts_at).toISOString(), ends_at: body.ends_at ? new Date(body.ends_at).toISOString() : null, location: String(body.location || '').trim() || null, created_by: req.session.user.id });
    }
    ...
    if (feature === 'welcome' && body.action === 'save') {
      ...
      await welcomeService.saveConfig(guildId, {
        ...
        enabled: true,
        auto_role_ids: roles,
        embed_data: { title: String(body.title || 'Willkommen!'), description: String(body.description || 'Willkommen auf {server}, {user}!'), color: String(body.color || '#2B3A67'), image: String(body.image || ''), thumbnail: String(body.thumbnail || '') },
      });
