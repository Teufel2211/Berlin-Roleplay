...
function registerEvents(client) {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    try {
      ...
      if (interaction.isButton()) { const id = interaction.customId; if (id.startsWith('pt:')) { const [, action, ticketId] = id.split(':'); if(action==='claim')return await proTicketService.claim(interaction,Number(ticketId));if(action==='unclaim')return await proTicketService.unclaim(interaction,Number(ticketId));if(action==='close')return await proTicketService.close(interaction,Number(ticketId));if(action==='reopen')return await proTicketService.reopen(interaction,Number(ticketId));if(action==='delete')return await proTicketService.deleteTicket(interaction,Number(ticketId));if(action==='transcript')return await proTicketService.transcript(interaction,Number(ticketId)); } if(id.startsWith('giveaway_leave_'))return await giveawayService.leaveGiveaway(interaction);if(id.startsWith('giveaway_cancel_'))return await giveawayService.leaveCancel(interaction);if(id.startsWith('giveaway_join_'))return await giveawayService.handleJoinButton(interaction);if(id.startsWith('mod_panel:'))return await moderationPanelService.handleButton(interaction);if(id.startsWith('verification'))return await verificationService.handleButton(interaction); }
      if (interaction.isStringSelectMenu()) { if(interaction.customId==='pt:category')return await proTicketService.categorySelect(interaction);if(interaction.customId==='mod_panel:select')return await moderationPanelService.handleSelect(interaction); }
      if (interaction.isModalSubmit()) { if(interaction.customId.startsWith('pt:questions:'))return await proTicketService.questionModal(interaction);if(interaction.customId.startsWith('mod_modal:'))return await moderationPanelService.handleModal(interaction); }
  ...
  client.on(Events.MessageCreate, async (message)=>{if(!message.guild||message.author.bot)return;try{const {data:ticket}=await withRetry(()=>getClient().from(TABLES.tickets).select('id,guild_id').eq('guild_id',message.guild.id).eq('channel_id',message.channel.id).maybeSingle());if(ticket){await withRetry(()=>getClient().from(TABLES.tickets).update({last_activity_at:message.createdAt.toISOString()}).eq('id',ticket.id));await withRetry(()=>getClient().from(TABLES.ticketMessages).insert({guild_id:message.guild.id,ticket_id:ticket.id,message_id:message.id,author_id:message.author.id,author_username:message.author.tag,content:message.content||'',attachments:[...message.attachments.values()].map(a=>({name:a.name,url:a.url,size:a.size})),embeds:message.embeds.map(e=>e.toJSON()),created_at:message.createdAt.toISOString()}));}}catch(err){logger.warn(`Ticket-Aktivität konnte nicht gespeichert werden: ${err.message}`);}});
  client.on(Events.GuildMemberRemove,async(member)=>{try{await getClient().from(TABLES.users).update({left_at:new Date().toISOString()}).eq('guild_id',member.guild.id).eq('discord_id',member.id);}catch(err){logger.warn(`users-Update bei MemberRemove fehlgeschlagen: ${err.message}`);}});
  client.on(Events.GuildMemberAdd, (member) => { welcomeService.handleMemberAdd(member).catch((err) => logger.warn(`Willkommen-Handler fehlgeschlagen: ${err.message}`)); });
}
