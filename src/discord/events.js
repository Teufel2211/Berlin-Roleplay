...
function registerEvents(client) {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    try {
      ...
      if (interaction.isButton()) {
        ...
        if (id === 'ticket_close') return await ticketService.showCloseModal(interaction);
      ...
      if (interaction.isModalSubmit()) {
        ...
        if (id === 'ticket_close_modal') return await ticketService.handleCloseModal(interaction);
        return;
      }
    } catch (err) {
  ...
  client.on(Events.GuildMemberRemove, async (member) => {
    try {
      await getClient().from(TABLES.users).update({ left_at: new Date().toISOString() }).eq('discord_id', member.id);
    } catch (err) {
