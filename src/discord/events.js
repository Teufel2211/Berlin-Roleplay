...
const COMMAND_MODULES = { giveaway: 'giveaway', ticket: 'tickets', moderation: 'moderation' };
...
function registerEvents(client) {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    try {
      ...
      if (interaction.isButton()) { const id = interaction.customId; if (id.startsWith('pt:')) { const [, action, ticketId] = id.split(':'); if(action==='claim')return await proTicketService.claim(interaction,Number(ticketId));if(action==='unclaim')return await proTicketService.unclaim(interaction,Number(ticketId));if(action==='close')return await proTicketService.close(interaction,Number(ticketId));if(action==='reopen')return await proTicketService.reopen(interaction,Number(ticketId));if(action==='delete')return await proTicketService.deleteTicket(interaction,Number(ticketId));if(action==='transcript')return await proTicketService.transcript(interaction,Number(ticketId)); } if(id==='ticket_panel')return await ticketService.handleOpen(interaction);if(id.startsWith('ticket_close_'))return await ticketService.showCloseModal(interaction);if(id.startsWith('ticket_claim_'))return await ticketService.claim(interaction);if(id.startsWith('giveaway_join_'))return await giveawayService.handleJoinButton(interaction);if(id.startsWith('mod_panel:'))return await moderationPanelService.handleButton(interaction); }
      if (interaction.isStringSelectMenu()) { if(interaction.customId==='pt:category')return await proTicketService.categorySelect(interaction);if(interaction.customId==='ticket_type_select')return await ticketService.handleTypeSelect(interaction);if(interaction.customId==='mod_panel:select')return await moderationPanelService.handleSelect(interaction); }
      if (interaction.isModalSubmit()) { if(interaction.customId.startsWith('pt:questions:'))return await proTicketService.questionModal(interaction);if(interaction.customId.startsWith('ticket_close_modal'))return await ticketService.handleCloseModal(interaction);if(interaction.customId.startsWith('mod_modal:'))return await moderationPanelService.handleModal(interaction); }
